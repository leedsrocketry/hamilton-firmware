...
#define SET_DATA \
  "AT+U"  // + data bits then check bits then stop bits -> N (no), O (odd), E
          // (even), 1 (1 stop bit), 2 (2 stop bits), 3 (1.5 stop bits)
...
/**
        @brief Configures the settings for the HC12, and returns its params to
  the ground station
