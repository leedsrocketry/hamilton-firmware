...
int8_t SI446_transmit() {
  ...
  SI446_command[2] = 00110000;         // go into ready state, send data currently in
                                       // FIFO, imeadiately
  ...
  SI446_retVal = SI446_check_CTS(-1);  // CTS returned once chip is in TX state
                                       // - doesnt mean packet has been sent
  spi_write_buf(SI446_SPI, SI446_command, 4);
