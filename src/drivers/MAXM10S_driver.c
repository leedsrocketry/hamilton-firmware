...
uint8_t PARSE_NAV_PVT(uint8_t *buf) {
  ...
  if (payload == NULL) {
    loge("Memory allocation failed for payload\n");
    return 1;  // Return error code
  }
  memcpy(payload, &buf[6], length);
  payload[length] = '\0';  // Null-terminate the payload for safety
  ...
  int32_t nano = payload[16] | (payload[17] << 8) | (payload[18] << 16) | (payload[19] <<
24);
  int32_t fixType = payload[20];
  ...
  logi("lat: %d, lon: %d, height: %d, hMSL: %d, numSV: %d\r\n", lat, lon, height, hMSL, numSV);
  free(payload);
}