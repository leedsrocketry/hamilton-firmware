...
uint8_t CRCAdd(uint8_t crc, uint8_t message_byte) { return CRCTable[(crc << 1) ^ message_byte]; }
...
uint8_t getCRC(uint8_t message[], uint8_t length) {
  ...
  for (i = 0; i < length; ++i) crc = CRCAdd(crc, message[i]);

  return crc;
}

SD_Card init_sd_card(SPI_TypeDef *spi_device, uint8_t chip_select) {
  SD_Card sd_card_dev = {.spi_interface = spi_device,
    ...
    if ((r7[3] & 0x1f) == 0x1) {
      ...
      if (r3[0] > 1) {
        ...
        if ((r3[2] & 0b00010000) || (r3[2] & 0b00100000)) {
          ...
          if (!broken) {
            LOG("Error - Failed to initialise SD card ACMD41 timed out card in "
                "idle state\r\n");
            return sd_card_dev;
          }
...
void select_sd_card(SD_Card *sd_device) { spi_enable_cs(sd_device->spi_interface, sd_device->chip_select); }

void deselect_sd_card(SD_Card *sd_device) { spi_disable_cs(sd_device->spi_interface, sd_device->chip_select); }
...
inline void sd_transmit_receive(SPI_TypeDef *spi, uint8_t *send_byte, uint8_t transmit_size, uint8_t receive_size,
                                uint8_t *result_ptr, uint8_t wait_cmds) {
  spi_ready_write(spi);
