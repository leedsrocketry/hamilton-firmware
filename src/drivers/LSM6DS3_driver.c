/*
    ...
    Description: Driver file for the IMU module LSM6DS3
   (https://www.mouser.co.uk/datasheet/2/389/dm00133076-1798402.pdf)

    ---------------------------------------
    Some basis for this logic comes from the open source Betaflight lsm6dso
   driver.
*/
...
void LSM6DS3_config(SPI_TypeDef *spi) {
  ...
  // LSM6DS3_write_register(spi, LSM6DSO_REG_INT1_CTRL, LSM6DSO_VAL_INT1_CTRL,
  // 1);

  // Disable interrupt pin 2
  // LSM6DS3_write_register(spi, LSM6DSO_REG_INT2_CTRL, LSM6DSO_VAL_INT2_CTRL,
  // 1);

  // Configure the accelerometer
  // 833hz ODR, 16G scale, use LPF1 output
  LSM6DS3_write_register(
      spi, LSM6DSO_REG_CTRL1_XL,
      (LSM6DSO_VAL_CTRL1_XL_ODR833 << 4) | (LSM6DSO_VAL_CTRL1_XL_16G << 2) | (LSM6DSO_VAL_CTRL1_XL_LPF1 << 1), 1);

  // Configure the gyro
  // 6664hz ODR, 2000dps scale
  LSM6DS3_write_register(spi, LSM6DSO_REG_CTRL2_G,
                         (LSM6DSO_VAL_CTRL2_G_ODR6664 << 4) | (LSM6DSO_VAL_CTRL2_G_2000DPS << 2), 1);

  // Configure control register 3
  // latch LSB/MSB during reads; set interrupt pins active high; set interrupt
  // pins push/pull; set 4-wire SPI; enable auto-increment burst reads
  LSM6DS3_write_register_bits(spi, LSM6DSO_REG_CTRL3_C, LSM6DSO_MASK_CTRL3_C,
  ...
  LSM6DS3_write_register_bits(spi, LSM6DSO_REG_CTRL4_C, LSM6DSO_MASK_CTRL4_C,
                              (LSM6DSO_VAL_CTRL4_C_I2C_DISABLE | LSM6DSO_VAL_CTRL4_C_LPF1_SEL_G), 1);

  // Configure control register 6
  // disable I2C interface; enable gyro LPF1
  LSM6DS3_write_register_bits(spi, LSM6DSO_REG_CTRL6_C, LSM6DSO_MASK_CTRL6_C,
                              (LSM6DSO_VAL_CTRL6_C_XL_HM_MODE | LSM6DSO_VAL_CTRL6_C_FTYPE_335HZ), 1);
...
bool LSM6DS3_acc_read(SPI_TypeDef *spi, LSM6DS3_data *gyro) {
  ...
  LSM6DS3_write_register(
      SPI1, LSM6DSO_REG_CTRL1_XL,
      (LSM6DSO_VAL_CTRL1_XL_ODR833 << 4) | (LSM6DSO_VAL_CTRL1_XL_16G << 2) | (LSM6DSO_VAL_CTRL1_XL_LPF1 << 1), 1);
  LSM6DS3_write_register(SPI1, LSM6DSO_REG_CTRL2_G,
                         (LSM6DSO_VAL_CTRL2_G_ODR6664 << 4) | (LSM6DSO_VAL_CTRL2_G_2000DPS << 2), 1);
  ...
  spi_enable_cs(spi, LSM6DS3_CS);
  delay_microseconds(1);
  spi_transmit_receive(spi, &(send_data), 1, 1,
                       &(lsm6ds3_rx_buf[0]));  // send read command and get first result
  ...
  gyro->x_accel =
      (int16_t)(LMS6DS6_ACCEL_SENSITIVITY *
                (int32_t)((int16_t)((lsm6ds3_rx_buf[IDX_ACCEL_XOUT_H] << 8) | lsm6ds3_rx_buf[IDX_ACCEL_XOUT_L])) /
                1000);
  gyro->y_accel =
      (int16_t)(LMS6DS6_ACCEL_SENSITIVITY *
                (int32_t)((int16_t)((lsm6ds3_rx_buf[IDX_ACCEL_YOUT_H] << 8) | lsm6ds3_rx_buf[IDX_ACCEL_YOUT_L])) /
                1000);
  gyro->z_accel =
      (int16_t)(LMS6DS6_ACCEL_SENSITIVITY *
                (int32_t)((int16_t)((lsm6ds3_rx_buf[IDX_ACCEL_ZOUT_H] << 8) | lsm6ds3_rx_buf[IDX_ACCEL_ZOUT_L])) /
                1000);
  // LOG("Accel: X:%6i, \tY:%6i,\tZ:%6i\r\n", gyro->x_accel, gyro->y_accel,
  // gyro->z_accel);
...
bool LSM6DS3_gyro_read(SPI_TypeDef *spi, LSM6DS3_data *gyro) {
  ...
  // LSM6DS3_write_register(SPI1, LSM6DSO_REG_CTRL2_G,
  // (LSM6DSO_VAL_CTRL2_G_ODR6664 << 4) | (LSM6DSO_VAL_CTRL2_G_2000DPS << 2),
  // 1);
  ...
  gyro->x_rate = LMS6DS6_ANGULAR_RATE_SENSITIVITY *
                     (int32_t)((int16_t)((lsm6ds3_rx_buf[IDX_GYRO_XOUT_H] << 8) | lsm6ds3_rx_buf[IDX_GYRO_XOUT_L])) -
                 gyro->x_offset;
  gyro->y_rate = LMS6DS6_ANGULAR_RATE_SENSITIVITY *
                     (int32_t)((int16_t)((lsm6ds3_rx_buf[IDX_GYRO_YOUT_H] << 8) | lsm6ds3_rx_buf[IDX_GYRO_YOUT_L])) -
                 gyro->y_offset;
  gyro->z_rate = LMS6DS6_ANGULAR_RATE_SENSITIVITY *
                     (int32_t)((int16_t)((lsm6ds3_rx_buf[IDX_GYRO_ZOUT_H] << 8) | lsm6ds3_rx_buf[IDX_GYRO_ZOUT_L])) -
                 gyro->z_offset;

  // printf("GryoR: X:%i, \tY:%i,\tZ:%i\r\n", gyro->x_rate, gyro->y_rate,
  // gyro->z_rate);
...
bool LSM6DS3_gyro_offsets(SPI_TypeDef *spi, LSM6DS3_data *gyro) {
  ...
  } while (!LSM6DS3_gyro_standard_dev(buff, LSM6DSO_OFFSET_BUFF_LEN,
                                      1000));  // if standard deviation of readings is not within limit then its
                                               // not steady enough & try again
